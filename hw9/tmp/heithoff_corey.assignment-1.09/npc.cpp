...
void npc_next_pos_rand(dungeon *d, npc *c, pair_t next)
{
  ...
  } while (!(c->characteristics & NPC_PASS_WALL && mappair(n) == ter_wall) && mappair(n) < ter_floor);
...
bool dungeon_boss_killed(dungeon *d)
{
  for(monster_description md : d->monster_descriptions){
    if(md.is_boss_killed()){
      return true;
    }
  }
  return false;
