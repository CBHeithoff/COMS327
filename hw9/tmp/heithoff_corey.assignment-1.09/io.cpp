...
static void inspect_item(dungeon *d, uint32_t carry_slot)
{
  clear();
  ...
  const std::string &desc = d->PC->inventory[carry_slot]->get_description();
  mvprintw(3, 1, "Inspecting Item: %s", d->PC->inventory[carry_slot]->get_name());
  ...
  while (words >> word) {
    if (line_buffer.length() + word.length() + 1 > max_width) {
      mvprintw(line++, 1, "%s", line_buffer.c_str());
      line_buffer = word;
    } else {
  ...
  mvprintw(line + 1, 1, "Hit escape to continue.");
  while (getch() != 27 /* escape */)
    ;
}

static void io_inventory_inspect(dungeon *d)
{
  uint32_t i;
  int c;
  char (*s)[41]; /* pointer to array of 40 char */

  s = (char(*)[41]) malloc(INVENTORY_SIZE * sizeof (*s));

  mvprintw(3, 19, " %-40s ", "");
  snprintf(s[0], 41, "Choose an item to inspect:");
  mvprintw(4, 19, " %-40s ", s[0]);
  mvprintw(5, 19, " %-40s ", "");

  for (i = 0; i < INVENTORY_SIZE; i++) {
    if(d->PC->inventory[i] != NULL){
      snprintf(s[i], 41, "%d.) %s", i, d->PC->inventory[i]->get_name());
    }
    else{
      snprintf(s[i], 41, "%d.)", i);
    }
  ...
  mvprintw(INVENTORY_SIZE + 6, 19, " %-40s ", "");
  mvprintw(INVENTORY_SIZE + 7, 19, " %-40s ", "Hit escape to cancel.");
...
static void io_inventory_expunge(dungeon *d)
{
  uint32_t i;
  int c;
  char (*s)[41]; /* pointer to array of 40 char */
  ...
  mvprintw(3, 19, " %-40s ", "");
  snprintf(s[0], 41, "Choose an item to expunge:");
  mvprintw(4, 19, " %-40s ", s[0]);
  mvprintw(5, 19, " %-40s ", "");
...
static void io_inventory_drop(dungeon *d)
{
  uint32_t i;
  int c;
  char (*s)[41]; /* pointer to array of 40 char */
  ...
  mvprintw(3, 19, " %-40s ", "");
  snprintf(s[0], 41, "Choose an item to drop:");
  mvprintw(4, 19, " %-40s ", s[0]);
  mvprintw(5, 19, " %-40s ", "");
...
static void io_inventory_wear(dungeon *d)
{
  uint32_t i;
  int c;
  char (*s)[41]; /* pointer to array of 40 char */
  ...
  mvprintw(3, 19, " %-40s ", "");
  snprintf(s[0], 41, "Choose an item to wear:");
  mvprintw(4, 19, " %-40s ", s[0]);
  mvprintw(5, 19, " %-40s ", "");
...
static void io_inventory(dungeon *d)
{
  uint32_t i;
  char (*s)[41]; /* pointer to array of 40 char */
  ...
  mvprintw(INVENTORY_SIZE + 6, 19, " %-40s ", "");
  mvprintw(INVENTORY_SIZE + 7, 19, " %-40s ", "Hit escape to continue.");
  while (getch() != 27 /* escape */)
    ;
...
static void io_equipment_take_off(dungeon *d)
{
  uint32_t i;
  int c;
  char (*s)[41]; /* pointer to array of 40 char */

  s = (char(*)[41]) malloc(num_equipment * sizeof (*s));

  mvprintw(3, 19, " %-40s ", "");
  snprintf(s[0], 41, "Choose an equipment to take off:");
  mvprintw(4, 19, " %-40s ", s[0]);
  mvprintw(5, 19, " %-40s ", "");

  for (i = 0; i < num_equipment; i++) {
    if(d->PC->equipment[i] != NULL){
      snprintf(s[i], 41, "%c.) %s", 'a' + i, d->PC->equipment[i]->get_name());
    }
    else{
      snprintf(s[i], 41, "%c.)", 'a' + i);
    }

    mvprintw(i + 6, 19, " %-40s ", s[i]);
  }

  mvprintw(num_equipment + 6, 19, " %-40s ", "");
  mvprintw(num_equipment + 7, 19, " %-40s ", "Hit escape to cancel.");

  do {
    c = getch();
    if (c >= 'a' && c <= 'l') {
      take_off_equipment_item(d, c - 'a');
    }
  } while (!(c >= 'a' && c <= 'l') && c != 27);
...
static void io_equipment(dungeon *d)
{
  uint32_t i;
  char (*s)[41]; /* pointer to array of 40 char */
  ...
  mvprintw(num_equipment + 6, 19, " %-40s ", "");
  mvprintw(num_equipment + 7, 19, " %-40s ", "Hit escape to continue.");
  while (getch() != 27 /* escape */)
    ;
...
static void inspect_monster(dungeon *d, pair_t dest)
{
  clear();
  ...
  if (dest[dim_y] == d->PC->position[dim_y] && dest[dim_x] == d->PC->position[dim_x]) {
    return;
  }

  npc *n = dynamic_cast<npc *>(d->character_map[dest[dim_y]][dest[dim_x]]);
  if (!n) {
    return;
  } 
  
  const std::string &desc = n->description;
  mvprintw(3, 1, "Inspecting Monster: %s", d->character_map[dest[dim_y]][dest[dim_x]]->name);
...
uint32_t io_inspect_monster(dungeon *d)
{
  ...
  mvprintw(0, 0,
           "Choose a location. 't' to inspect monster. ESC to cancel.");
...
void io_handle_input(dungeon *d)
{
  ...
  do {
    ...
    case 'd':
      ...
      break;
    case 'x':
      io_inventory_expunge(d);
      fail_code = 1;
      break;
    case 'I':
      io_inventory_inspect(d);
      fail_code = 1;
      break;
    case 'e':
      io_equipment(d);
