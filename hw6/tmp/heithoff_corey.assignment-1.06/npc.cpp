...
void npc::gen_monsters(dungeon_t *d)
{
  uint32_t i;
  character_t *m;
  uint32_t room;
  ...
  for (i = 0; i < d->num_monsters; i++) {
    m = (character_t *) malloc(sizeof (npc_t));
    memset(m, 0, sizeof (npc_t));
    ...
    // npc pointer is malloced but never used for game except when at the end of the game when freeing monsters
    m->npc = (npc *) malloc(sizeof (*m->npc));
    ((npc*)m)->characteristics = rand() & 0x0000000f;
    m->symbol = symbol[((npc*)m)->characteristics];
    ((npc*)m)->have_seen_pc = 0;
...
void npc_next_pos_gradient(dungeon_t *d, character_t *c, pair_t next)
{
  ...
  pair_t min_next;
  uint16_t min_cost;
  if (((npc*)c)->characteristics & NPC_TUNNEL) {
...
static void npc_next_pos_00(dungeon_t *d, character_t *c, pair_t next)
{
  ...
  if (can_see(d, c, d->pc)) {
    ((npc*)c)->pc_last_known_position[dim_y] = d->pc->position[dim_y];
    ((npc*)c)->pc_last_known_position[dim_x] = d->pc->position[dim_x];
...
static void npc_next_pos_01(dungeon_t *d, character_t *c, pair_t next)
{
  ...
  if (can_see(d, c, d->pc)) {
    ((npc*)c)->pc_last_known_position[dim_y] = d->pc->position[dim_y];
    ((npc*)c)->pc_last_known_position[dim_x] = d->pc->position[dim_x];
    ((npc*)c)->have_seen_pc = 1;
    npc_next_pos_line_of_sight(d, c, next);
  } else if (((npc*)c)->have_seen_pc) {
    npc_next_pos_line_of_sight(d, c, next);
  }

  if (((npc*)c)->have_seen_pc &&
      (next[dim_x] == ((npc*)c)->pc_last_known_position[dim_x]) &&
      (next[dim_y] == ((npc*)c)->pc_last_known_position[dim_y])) {
      ((npc*)c)->have_seen_pc = 0;
  }
}
...
static void npc_next_pos_erratic(dungeon_t *d, character_t *c, pair_t next)
{
  ...
  if (rand() & 1) {
    ...
    npc_move_func[((npc*)c)->characteristics & 0x00000007](d, c, next);
  }
}

void npc::npc_next_pos(dungeon_t *d, character_t *c, pair_t next)
{
  next[dim_y] = c->position[dim_y];
  next[dim_x] = c->position[dim_x];
  
  npc_move_func[((npc*)c)->characteristics & 0x0000000f](d, c, next);
}

uint32_t npc::dungeon_has_npcs(dungeon_t *d)
{
