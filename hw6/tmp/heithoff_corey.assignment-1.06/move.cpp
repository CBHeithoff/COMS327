...
uint32_t move_pc_teleport(dungeon_t *d, uint32_t dir)
{
  // Very similar to move_pc() but for individual movements while moving teleporting target pointer
  ...
  if ((mappair(next) >= ter_floor) || mappair(next) == ter_wall) {
    d->pc->position[dim_y] = next[dim_y];
    d->pc->position[dim_x] = next[dim_x];
    io_display_teleport(d);

  } else if (mappair(next) < ter_floor && mappair(next) != ter_wall) {
    io_queue_message(wallmsg[rand() % (sizeof (wallmsg) /
                                       sizeof (wallmsg[0]))]);
    io_display_teleport(d);
  }
...
uint32_t teleport_pc(dungeon_t *d, pair_t start)
{
  // Very similar to move_character() for final teleporting step
  
  pair_t dist; 
  dist[dim_y] = d->pc->position[dim_y];
  dist[dim_x] = d->pc->position[dim_x];

  d->pc->position[dim_y] = start[dim_y];
  d->pc->position[dim_x] = start[dim_x];

  if (mappair(dist) < ter_floor) {
    mappair(dist) = ter_floor;
  }

  //move_character(d, d->pc, dist);
  if (charpair(dist) &&
  ((dist[dim_y] != d->pc->position[dim_y]) ||
   (dist[dim_x] != d->pc->position[dim_x]))) {
    do_combat(d, d->pc, charpair(dist));
  }

  d->character[d->pc->position[dim_y]][d->pc->position[dim_x]] = NULL;
  d->pc->position[dim_y] = dist[dim_y];
  d->pc->position[dim_x] = dist[dim_x];
