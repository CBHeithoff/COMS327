  ...
  while ((p = (corridor_path_t *) heap_remove_min(&h))) {
    ...
    if ((p->pos[dim_y] == to[dim_y]) && p->pos[dim_x] == to[dim_x]) {
      for (x = to[dim_x], y = to[dim_y];
           ((int64_t) x != (int64_t) from[dim_x]) || ((int64_t) y != (int64_t) from[dim_y]);
...
static int create_cycle(dungeon_t *d)
{
  ...
  for (i = max = 0; (int64_t) i < (int64_t) d->num_rooms - 1; i++) {
    for (j = i + 1; (int64_t) j < (int64_t) d->num_rooms; j++) {
...
int dungeon::gen_dungeon(dungeon_t *d)
{
  empty_dungeon(d);
...
void dungeon::delete_dungeon(dungeon_t *d)
{
  free(d->rooms);
...
void dungeon::init_dungeon(dungeon_t *d)
{
  empty_dungeon(d);
...
int dungeon::write_dungeon(dungeon_t *d, char *file)
{
  const char *home;
  char *filename;
  FILE *f;
...
int read_rooms(dungeon_t *d, FILE *f)
{
  ...
  for (i = 0; i < d->num_rooms; i++) {
    ...
    for (y = d->rooms[i].position[dim_y];
        (int64_t) y < (int64_t) (d->rooms[i].position[dim_y] + d->rooms[i].size[dim_y]);
         y++) {
      for (x = d->rooms[i].position[dim_x];
          (int64_t) x < (int64_t) (d->rooms[i].position[dim_x] + d->rooms[i].size[dim_x]);
           x++) {
...
int dungeon::read_dungeon(dungeon_t *d, char *file)
{
...
int dungeon::read_pgm(dungeon_t *d, char *pgm)
{
  FILE *f;
  char s[80];
...
void dungeon::new_dungeon(dungeon_t *d)
{
