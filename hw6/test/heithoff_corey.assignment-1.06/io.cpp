...
void io_display_dark(dungeon_t *d)
{
  uint32_t y, x;
  character_t *c;
  ...
  for (y = 0; y < 21; y++) {
    for (x = 0; x < 80; x++) {
      if((int64_t) y > (int64_t)d->pc->position[dim_y]-3 && 
        (int64_t)y < (int64_t)d->pc->position[dim_y]+3 &&
        (int64_t)x > (int64_t)d->pc->position[dim_x]-3 && 
        (int64_t)x < (int64_t)d->pc->position[dim_x]+3 &&
        (d->character[y][x])){
...
void io_display_teleport(dungeon_t *d)
{
  ...
  for (y = 0; y < 21; y++) {
    for (x = 0; x < 80; x++) {
      if ((int64_t) y == (int64_t) d->pc->position[dim_y] && (int64_t) x == (int64_t) d->pc->position[dim_x]) {
        mvaddch(y + 1, x, '*');
...
uint32_t io_teleport_pc_rand(dungeon_t *d)
{
  /* Just for fun. */
  pair_t dest;
...
uint32_t io_teleport_pc(dungeon_t *d)
{
  ...
  start[dim_y] = d->pc->position[dim_y];
  start[dim_x] = d->pc->position[dim_x];
  ...
  do {
    ...
    case 'y':
    case KEY_HOME:
      fail_code = move_pc_teleport(d, 7);
      break;
    case '8':
    case 'k':
    case KEY_UP:
      fail_code = move_pc_teleport(d, 8);
      break;
    case '9':
    case 'u':
    case KEY_PPAGE:
      fail_code = move_pc_teleport(d, 9);
      break;
    case '6':
    case 'l':
    case KEY_RIGHT:
      fail_code = move_pc_teleport(d, 6);
      break;
    case '3':
    case 'n':
    case KEY_NPAGE:
      fail_code = move_pc_teleport(d, 3);
      break;
    case '2':
    case 'j':
    case KEY_DOWN:
      fail_code = move_pc_teleport(d, 2);
      break;
    case '1':
    case 'b':
    case KEY_END:
      fail_code = move_pc_teleport(d, 1);
      break;
    case '4':
    case 'h':
    case KEY_LEFT:
      fail_code = move_pc_teleport(d, 4);
      break;
    case '5':
    case ' ':
    case '.':
