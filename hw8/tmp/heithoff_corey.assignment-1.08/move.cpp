...
static void new_dungeon_level(dungeon *d, uint32_t dir)
{
  ...
  case '<':
  case '>':
    // allowing UNIQ monsters have only one existance while alive
    for(int y = 0; y < DUNGEON_Y; y++){
      for (int x = 0; x < DUNGEON_X; x++) {
        if (d->character_map[y][x]){
          if(d->character_map[y][x] != d->PC && 
             d->character_map[y][x]->alive &&
             ((npc*)d->character_map[y][x])->characteristics & NPC_UNIQ){
              d->unique_monsters[ ((npc*)d->character_map[y][x])->mon_desc_id] = 0;
          }
        }
        // allowing artifact objects to have only one existance
        if (d->object_map[y][x]){
          if(!d->object_map[y][x]->destroyed &&
            d->object_descriptions[d->object_map[y][x]->obj_desc_id].get_artifact()){
            d->artifact_objects[d->object_map[y][x]->obj_desc_id] = 0;
          }
        }
      }
    }
