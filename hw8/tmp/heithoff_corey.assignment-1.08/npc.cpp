...
void gen_monsters(dungeon *d)
{
  ...
  for (i = 0; i < d->num_monsters; i++) {
    ...
    for (size_t j = 0; j < d->unique_monsters.size(); ++j) {
        if (d->unique_monsters[j] != 1) {
          all_unique = false;
            break;
        }
    }
    ...
    // determining the monster template for the monster
    int mon_desc_id;
    do {
      mon_desc_id = rand_range(0, d->monster_descriptions.size() - 1);
    } while (d->unique_monsters[mon_desc_id] || 
            (uint32_t)rand_range(0, 99) >= d->monster_descriptions[mon_desc_id].get_rarity());

    // determining position of monster
    do {
    ...
    // determining monster details
    m->mon_desc_id = mon_desc_id;
    d->character_map[p[dim_y]][p[dim_x]] = m;
    m->speed = d->monster_descriptions[mon_desc_id].get_speed().roll(); //rand_range(5, 20);
    m->hitpoints = d->monster_descriptions[mon_desc_id].get_hitpoints().roll();
    m->color = d->monster_descriptions[mon_desc_id].get_color();
    m->alive = 1;
    m->sequence_number = ++d->character_sequence_number;
    m->characteristics = (npc_characteristics_t)(d->monster_descriptions[mon_desc_id].get_abilities()); //rand() & 0x0000000f;
    if(m->characteristics & NPC_UNIQ){
      d->unique_monsters[mon_desc_id] = 1;
    }
    /*    m->npc->characteristics = 0xf;*/
    m->symbol = d->monster_descriptions[mon_desc_id].get_symbol(); //symbol[m->characteristics];
    m->have_seen_pc = 0;
