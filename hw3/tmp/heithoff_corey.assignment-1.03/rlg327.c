...
/* Path finding for non-tunneling monsters. Modified version of the * 
 * dijkstra_corridor function above                                 */
static void dijkstra_path(dungeon_t *d)
{
  // initialization
  static corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  ...
  // initializaing the path map's positions
  if (!initialized) {
    for (y = 0; y < DUNGEON_Y; y++) {
      for (x = 0; x < DUNGEON_X; x++) {
        path[y][x].pos[dim_y] = y;
  ...
  // initializaing the path map's costs and finding the PC
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      path[y][x].cost = INT_MAX;
      if(d->map[y][x] == ter_pc){
  ...
  // initializaing the source location
  path[from[dim_y]][from[dim_x]].cost = 0;

  // initializaing heap
  heap_init(&h, corridor_path_cmp, NULL);

  // inserting needed nodes/cells, including all rooms and corridors
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    // determining and comparing cost of 8 surrounding nodes
    // all cell weights are 1
  ...
  // Printing out the non-tunneling distance map
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      if(mapxy(x, y) != ter_wall_immutable && mapxy(x, y) != ter_wall && mapxy(x, y) != ter_pc){
        if(path[y][x].cost == INT_MAX || path[y][x].cost < 0){
          printf("%c", 'X'); 
        }else{
          printf("%d", (path[y][x].cost % 10));  
        }
      }else if(mapxy(x, y) == ter_pc){
        printf("%c", '@');
      }else{
        printf("%c", ' ');
      }
...
/* Path finding for tunneling monsters. Modified version of the * 
 * dijkstra_corridor function above                             */
static void dijkstra_path_tunneling(dungeon_t *d)
{
  // initialization
  static corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      if(mapxy(x, y) != ter_wall_immutable && mapxy(x, y) != ter_pc){
        printf("%d", (path[y][x].cost % 10));  
      }else if(mapxy(x, y) == ter_pc){
        printf("%c", '@');
      }else{
...
void place_pc(dungeon_t *d)
{
  ...
  i = rand_range(0, d->num_rooms - 1);
  r = d->rooms + i;
  e1[dim_y] = rand_range(r->position[dim_y],
                         r->position[dim_y] + r->size[dim_y] - 1);
  e1[dim_x] = rand_range(r->position[dim_x],
                         r->position[dim_x] + r->size[dim_x] - 1);
  
  d->map[e1[dim_y]][e1[dim_x]] = ter_pc;
}
...
int main(int argc, char *argv[])
{
  ...
  // printing dungeon terrain map
  render_dungeon(&d);

  // printing dungeon distance maps
  dijkstra_path(&d);
  printf("\n\n"); // spacing
  dijkstra_path_tunneling(&d);
