...
void pc::recalculate_speed()
{
  ...
  for (speed = PC_SPEED, weight = 0, i = 0; i < num_eq_slots; i++) {
    if (eq[i]) {
      speed += eq[i]->get_speed();
      weight += eq[i]->get_weight();
    }
  }
  for (i = 0; i < MAX_INVENTORY; i++) {
    if (in[i]) {
      weight += in[i]->get_weight();
    }
  }
  speed -= weight / 4;
...
uint32_t pc::sell_in(dungeon *d, uint32_t slot)
{
  if (!in[slot] || !in[slot]->is_destructable()) {
    return 1;
  }

  io_queue_message("You sold %s.", in[slot]->get_name());

  d->PC->value += in[slot]->get_value();
...
uint32_t pc::terrain_eff(dungeon *d)
{
  pair_t next;
  uint32_t tele_found;
  ...
  // handling teleporter
  if(specmappair(d->PC->position) == ter_teleport){
    tele_found = 0;
    for (next[dim_y] = 0; next[dim_y] < DUNGEON_Y && !tele_found; next[dim_y]++) {
      for (next[dim_x] = 0; next[dim_x] < DUNGEON_X && !tele_found; next[dim_x]++) {
        if (specmappair(next) == ter_teleport && 
  ...
  // handling slowing terrain
  else if(specmappair(d->PC->position) == ter_quicksand ||
          specmappair(d->PC->position) == ter_vines){
    if(d->PC->speed > 1){
      d->PC->speed /= 2;
    }
  }
  // handling damaging terrain
  else if(specmappair(d->PC->position) == ter_lava_pit){
    d->PC->hp -= 25;
  }
  // handling healing terrain
  else if(specmappair(d->PC->position) == ter_healing_pool){
    if(d->PC->hp <= PC_STARTING_HP - 10){
      d->PC->hp += 10;
    }
    else if(d->PC->hp <= PC_STARTING_HP){
      d->PC->hp = PC_STARTING_HP;
    }
  }
