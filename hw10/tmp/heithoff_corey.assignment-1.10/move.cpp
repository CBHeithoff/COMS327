...
void do_combat(dungeon *d, character *atk, character *def)
{
  ...
  if (character_is_alive(def)) {
    if (atk != d->PC) {
      ...
      if(rand_range(0, 150) < dodge){
        damage = 0;
        io_queue_message("%s%s attacked but you dodged.", is_unique(atk) ? "" : "The ", atk->name);
      }else{
        io_queue_message("%s%s %s your %s for %d but you deflected %d.", is_unique(atk) ? "" : "The ",
                        atk->name, attacks[rand() % (sizeof (attacks) /
                                                      sizeof (attacks[0]))],
                        organs[rand() % (sizeof (organs) /
                                          sizeof (organs[0]))], damage, (damage < defence) ? damage : defence);
      ...
      for (i = 0; i < num_eq_slots; i++) {
        if (d->PC->eq[i]) {
          hit += d->PC->eq[i]->get_hit();
        }
      }
      if(rand_range(0, 100) < hit){
        io_queue_message("You hit %s%s for %d.", is_unique(def) ? "" : "the ",
                        def->name, damage);
      }else{
        damage = 0;
        io_queue_message("You missed %s%s.", is_unique(def) ? "" : "the ",
                        def->name);
      }
    }
