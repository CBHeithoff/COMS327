...
npc *monster_description::generate_monster(dungeon *d)
{
  ...
  while (!v[(i = (rand() % v.size()))].can_be_generated() ||
         ...
         (v[i].is_monster_boss() && d->level != 10) ||
         (!v[i].is_high_diff_monster() && d->level > 6 && !v[i].is_monster_boss()) ||
         (v[i].is_high_diff_monster() && d->level < 5) ||
         (!v[i].is_fast_monster() && d->level == 6))
    ;

    // high diff monsters: titan, grass mud horse, amazon Lich Queen, software bug, and C++ program
    // fast monsters: software bugs, river crab, and Rory Gilmore
