...
static void rand_seed(time_t seed)
{
  srand(seed);
}

static void random_rand_seed()
{
  time_t seed;
  struct timeval tv;
...
{
  pair_t p, check;
  uint32_t i, j, uniq_tele;
  special_terrain_t spec_ter;
  pair_t five_wide[13] = {
    { -1, -1 },
    { -1,  0 },
    { -1,  1 },
  ...
  // special terrain only occurs below dungeon level 2
  if(d->level > 2){
    // creating 3 to 5 special terrain areas
    for(i = rand_range(3,5); i > 0; i--){
      // getting initial location
      do {
        p[dim_y] = rand_range(1, DUNGEON_Y - 2);
        p[dim_x] = rand_range(1, DUNGEON_X - 2);
      } while ((mappair(p) < ter_floor) ||
               (specmappair(p) > ter_normal) ||
               (mappair(p) != ter_floor_room && rand_range(0,1)));
      
      // choosing random special terrain
      do {
        spec_ter = (special_terrain_t)rand_range(1, ter_amount - 1);
      } while ((spec_ter == ter_teleport && uniq_tele) || spec_ter == ter_fog);
      
      // placing special terrain area for 5 wide areas
      if(spec_ter == ter_quicksand || 
         spec_ter == ter_lava_pit ||
         spec_ter == ter_vines ||
         spec_ter == ter_fog){
        for(j = 0; j < 13; j++){
          check[dim_y] = p[dim_y] + five_wide[j][dim_y];
          check[dim_x] = p[dim_x] + five_wide[j][dim_x];
          if((mappair(check) > ter_floor) &&
             (specmappair(check) == ter_normal)){
            specmappair(check) = spec_ter;
          }
        }
      }
      // placing special terrain area for 3 wide areas
      else if(spec_ter == ter_healing_pool){
        for(j = 0; j < 5; j++){
          check[dim_y] = p[dim_y] + three_wide[j][dim_y];
          check[dim_x] = p[dim_x] + three_wide[j][dim_x];
          if((mappair(check) > ter_floor) &&
             (specmappair(check) == ter_normal)){
            specmappair(check) = spec_ter;
          }
      ...
      // placing teleporting special terrain
      else if(spec_ter == ter_teleport){
        uniq_tele = 1;
        specmappair(p) = spec_ter;
        do {
          check[dim_y] = rand_range(1, DUNGEON_Y - 2);
          check[dim_x] = rand_range(1, DUNGEON_X - 2);
        } while ((mappair(check) < ter_floor) ||
                 (mappair(check) > ter_stairs) ||
                 (specmappair(check) > ter_normal));
        specmappair(check) = spec_ter;
      }
    }
...
void new_dungeon(dungeon *d, bool down)
{
  ...
  new_level = d->seed + d->level;
  down ? new_level += 1 : new_level -= 1;
  down ? d->level += 1 : d->level -= 1;
  rand_seed(new_level);
  ...
  if(new_level != d->seed){
    ...
    gen_monsters(d);
    gen_objects(d);
    gen_special_terrain(d);
  }else{
    d->PC->position[dim_y] = 10;
    d->PC->position[dim_x] = 40;
    d->character_map[d->PC->position[dim_y]][d->PC->position[dim_x]] = d->PC;
    town_dungeon(d);
  }
}
