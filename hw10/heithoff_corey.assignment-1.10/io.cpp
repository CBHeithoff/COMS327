...
#define spec_ter_color(color) { COLOR_##color }
...
static void io_redisplay_PC(dungeon *d)
{
  uint32_t color;
  attron(COLOR_PAIR((color = d->PC->get_color())));
  mvaddch(d->PC->position[dim_y] + 1, d->PC->position[dim_x], d->PC->get_symbol());
  ...
  for (pos[dim_y] = -PC_VISUAL_RANGE;
    ...
    for (pos[dim_x] = -PC_VISUAL_RANGE;
      ...
      if (cursor[dim_y] == d->PC->position[dim_y] + pos[dim_y] &&
        ...
        case ter_floor:
        case ter_floor_room:
          if(specmapxy(d->PC->position[dim_x] + pos[dim_x],
                    d->PC->position[dim_y] + pos[dim_y]) > ter_fog){
            attron(COLOR_PAIR(special_terrain_color[specmapxy(d->PC->position[dim_x] + pos[dim_x],
                    d->PC->position[dim_y] + pos[dim_y])]));
            mvaddch(d->PC->position[dim_y] + pos[dim_y] + 1,
                    d->PC->position[dim_x] + pos[dim_x], '.');
            attroff(COLOR_PAIR(special_terrain_color[specmapxy(d->PC->position[dim_x] + pos[dim_x],
                    d->PC->position[dim_y] + pos[dim_y])]));
          }
          else{
            mvaddch(d->PC->position[dim_y] + pos[dim_y] + 1,
  ...
  for (visible_monsters = -1, pos[dim_y] = 0;
    ...
    for (pos[dim_x] = 0; pos[dim_x] < DUNGEON_X; pos[dim_x]++) {
      ...
      if (d->character_map[pos[dim_y]]
        ...
        case ter_floor:
        case ter_floor_room:
          if(specmappair(pos) > ter_fog){
            attron(COLOR_PAIR(special_terrain_color[specmappair(pos)]));
            mvaddch(pos[dim_y] + 1, pos[dim_x], '.');
            attroff(COLOR_PAIR(special_terrain_color[specmappair(pos)]));
          }
          else{
            mvaddch(pos[dim_y] + 1, pos[dim_x], '.');
          }
...
void io_display_town(dungeon *d)
{
  uint32_t y, x;
  uint32_t color;
  character *c;
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      if (d->PC->position[dim_y] == (int16_t) y && d->PC->position[dim_x] == (int16_t) x) {
        attron(COLOR_PAIR((color = d->PC->get_color())));
        mvaddch(d->PC->position[dim_y] + 1, d->PC->position[dim_x], d->PC->get_symbol());
        attroff(COLOR_PAIR(color));
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      if (d->character_map[y][x]) {
        ...
        case ter_floor:
        case ter_floor_room:
          if(specmapxy(x,y) > ter_fog){
            attron(COLOR_PAIR(special_terrain_color[specmapxy(x,y)]));
            mvaddch(y + 1, x, '.');
            attroff(COLOR_PAIR(special_terrain_color[specmapxy(x,y)]));
          }
          else{
            mvaddch(y + 1, x, '.');
          }
...
{
  mvprintw(11, 30, " HP:    %7d ", d->PC->hp);
  mvprintw(12, 30, " Speed: %7d ", d->PC->speed);
  mvprintw(13, 30, " Gold:  %7d ", d->PC->value);
  mvprintw(14, 30, " Dungeon        ");
  mvprintw(15, 30, " Level: %7d ", d->level);
  mvprintw(17, 27, " Hit any key to continue. ");
  refresh();
  getch();
  if(!d->level){
    io_display_town(d);
...
void io_object_to_string_full(object *o, char *s, uint32_t size)
{
  if (o) {
    snprintf(s, size, "Stats: (hit: %d, dodge: %d, def: %d, lbs: %d, val: %d, attr: %d)",
             o->get_hit(), o->get_dodge(),
             o->get_defence(), o->get_weight(),
             o->get_value(), o->get_attribute());
  } else {
    *s = '\0';
  }
}
...
static uint32_t io_display_obj_info(object *o)
{
  ...
  mvprintw(1, 0, s);
  io_object_to_string_full(o, s, 80);
  mvprintw(2, 0, s);
  mvprintw(4, 0, o->get_description());

  mvprintw(n + 6, 0, "Hit any key to continue.");
...
static uint32_t io_inspect_monster(dungeon *d)
{
  ...
  * not otherwise used.                                                 */
  ...
  snprintf(s, 80,"%s, %d speed, %d HP, %d+%dd%d damage, %d gold                                  ",
           charpair(dest)->name,
           charpair(dest)->speed,
           charpair(dest)->hp,
...
uint32_t io_misc_shop(dungeon *d)
{
  uint32_t i, j, index, counter, key;
  char s[61];

  mvprintw(4, 10, " Your Gold: %d ", d->PC->value);

  i = index = 0;
  while (index < d->object_descriptions.size()) {
    /* We'll write some lines, some weapons, 1 blank, and 1 prompt.  *
     * We'll limit width to 60 characters, so very long object names *
     * will be truncated.  In an 80x24 terminal, this gives offsets  *
     * at 10 x and 6 y to start printing things.                     */
    if(d->object_descriptions[index].is_misc_shop()) {
      mvprintw(i + 6, 10, " %c) [%d] %-48s ", '0' + i, 
                (d->object_descriptions[index].get_value().get_base() + 
                d->object_descriptions[index].get_value().get_number() *
                d->object_descriptions[index].get_value().get_sides()), 
                d->object_descriptions[index].get_name().c_str());
      i++;
    } 
    index++;
  }
  mvprintw(i + 6, 10, " %-58s ", "");
  mvprintw(i + 7, 10, " %-58s ", "Purchase which item (ESC to cancel)?");
  refresh();
  ...
  while (1) {
    ...
    if (key < '0' || key > ('0' + i - 1)) {
      if (isprint(key)) {
        snprintf(s, 61, "Invalid input: '%c'.  Enter 0-%d or ESC to cancel.", key, i-1);
        mvprintw(i + 9, 10, " %-58s ", s);
      } else {
        snprintf(s, 61, "Invalid input.  Enter 0-%d or ESC to cancel.", i-1);
        mvprintw(i + 9, 10, " %-58s ", s);
      }
      refresh();
      continue;
    ...
    for(j = 0, counter = 0; j < d->object_descriptions.size(); j++) {
      if(d->object_descriptions[j].is_misc_shop()) {
        if(counter == key - '0'){
          break;
        }
        counter++;
    ...
    if ((d->object_descriptions[j].get_value().get_base() + 
          d->object_descriptions[j].get_value().get_number() *
          d->object_descriptions[j].get_value().get_sides()) > (int32_t) d->PC->value) {
      mvprintw(i + 9, 10, " %-58s ", "Not enough funds.  Try again.");
      continue;
    }
    
    // debugging
    //io_queue_message("buying %d.", j);

    if (!buy_object(d, j)) {
      io_queue_message("You bought %s.", d->object_descriptions[j].get_name().c_str());
      io_display_town(d);
      return 1;
    }

    mvprintw(i + 9, 10, " %-58s ", "Cannot purchase this artifact.  Try again.");
    refresh();
  }
...
uint32_t io_armor_shop(dungeon *d)
{
  uint32_t i, j, index, counter, key;
  char s[61];
  ...
  while (index < d->object_descriptions.size()) {
     ...
     * will be truncated.  In an 80x24 terminal, this gives offsets  *
     * at 10 x and 6 y to start printing things.                     */
    if(d->object_descriptions[index].is_armor_shop()) {
      mvprintw(i + 6, 10, " %c) [%d] %-48s ", '0' + i, 
                (d->object_descriptions[index].get_value().get_base() + 
                d->object_descriptions[index].get_value().get_number() *
                d->object_descriptions[index].get_value().get_sides()), 
  ...
  while (1) {
    ...
    for(j = 0, counter = 0; j < d->object_descriptions.size(); j++) {
      if(d->object_descriptions[j].is_armor_shop()) {
        if(counter == key - '0'){
          break;
        }
        counter++;
...
uint32_t io_weapon_shop(dungeon *d)
{
  uint32_t i, j, index, counter, key;
  char s[61];
  ...
  while (index < d->object_descriptions.size()) {
     ...
     * will be truncated.  In an 80x24 terminal, this gives offsets  *
     * at 10 x and 6 y to start printing things.                     */
    if(d->object_descriptions[index].is_weapon_shop()) {
      mvprintw(i + 6, 10, " %c) [%d] %-48s ", '0' + i, 
                (d->object_descriptions[index].get_value().get_base() + 
                d->object_descriptions[index].get_value().get_number() *
                d->object_descriptions[index].get_value().get_sides()), 
  ...
  while (1) {
    ...
    for(j = 0, counter = 0; j < d->object_descriptions.size(); j++) {
      if(d->object_descriptions[j].is_weapon_shop()) {
        if(counter == key - '0'){
          break;
        }
        counter++;
...
uint32_t io_sell_in(dungeon *d)
{
  ...
  mvprintw(16, 10, " %-58s ", "");
  mvprintw(17, 10, " %-58s ", "Sell which item (ESC to cancel)?");
  refresh();
  ...
  while (1) {
    ...
    if (!d->PC->sell_in(d, key - '0')) {
      io_display_town(d);
      return 1;
    }

    snprintf(s, 61, "Can't sell %s.  Try again.",
             d->PC->in[key - '0']->get_name());
    mvprintw(18, 10, " %-58s ", s);
    refresh();
  }
...
uint32_t enter_shop(dungeon *d)
{
  if(d->level != 0){
    return 0;
  }
  if(d->PC->position[dim_y] == 5 && d->PC->position[dim_x] == 11){
    return io_armor_shop(d);
  }else if (d->PC->position[dim_y] == 7 && d->PC->position[dim_x] == 59){
    return io_misc_shop(d); 
  }else if (d->PC->position[dim_y] == 16 && d->PC->position[dim_x] == 19){
    return io_weapon_shop(d);
  }else if (d->PC->position[dim_y] == 17 && d->PC->position[dim_x] == 53){
    return io_sell_in(d);
  }
