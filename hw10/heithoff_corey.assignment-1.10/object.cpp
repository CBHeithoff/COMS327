...
uint32_t buy_object(dungeon *d, uint32_t index)
{
  object *o;
  pair_t p;
  std::vector<object_description> &v = d->object_descriptions;

  if(!v[index].can_be_generated()){
    return 1;
  }

  p[dim_y] = d->PC->position[dim_y] + 1;
  p[dim_x] = d->PC->position[dim_x] + 1;

  o = new object(v[index], p, d->objmap[p[dim_y]][p[dim_x]]);
  d->objmap[p[dim_y]][p[dim_x]] = o;

  d->PC->value -= (d->object_descriptions[index].get_value().get_base() + 
