...
void move_character(dungeon_t *d, character_t *c, pair_t next)
{
  ...
  if (charpair(next) &&
    ...
    render_cell_ncurses(d, c->position);
  } else {
  ...
  render_cell_ncurses(d, next);
}

uint32_t do_moves(dungeon_t *d)
{
  pair_t next;
  character_t *c;
  event_t *e;
  uint32_t pc_special_input = 0;
  ...
  if (pc_is_alive(d) && e->c == &d->pc) {
    ...
    event_delete(e);
    //pc_next_pos(d, next);
    pc_special_input = pc_next_pos_input(d, next);
    next[dim_x] += c->position[dim_x];
    next[dim_y] += c->position[dim_y];
    // validating PC movement
    if(next[dim_x] > DUNGEON_X - 2 || next[dim_x] < 1 || 
       next[dim_y] > DUNGEON_Y - 2 || next[dim_y] < 1 ||
       mappair(next) <= ter_floor){
      next[dim_x] = c->position[dim_x];
      next[dim_y] = c->position[dim_y];
      mvprintw(0, 0, "There's a wall in the way!");
      refresh();
    }
    // moving up or down a staircase
    if (pc_special_input == 1 && mappair(next) > ter_stairs){
      return pc_special_input;
    }
    // exiting
    if (pc_special_input == 255){
      return pc_special_input;
    }
    // for PC tunneling
