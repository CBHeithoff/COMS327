...
void delete_dungeon(dungeon_t *d)
{
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
        ...
        // if (d->characters[y][x] != NULL) {
        //   if(((monster_t *)d->characters[y][x])->sequence_num > 0){
        //     free(d->characters[y][x]); 
        //   }
        //   //free(d->characters[y][x]);  
        // }
...
int gen_characters(dungeon_t *d, uint32_t nummon)
{
  uint32_t i;
  monster_t *tmp;
  pair_t p;
  ...
  // initializing PC in character table
  d->pc.sequence_num = 0;
  d->pc.next_turn = 0;
  d->pc.speed = PC_SPEED;
  //d->characters[d->pc.position[dim_y]][d->pc.position[dim_x]] = (void*) &(d->pc);

  // DEBUGGING
  // printf("PC is at (y, x): %d, %d\n",
  //        d->pc.position[dim_y], d->pc.position[dim_x]);
  // printf("PC is at (y, x): %d, %d\n",
  //   ((pc_t*)(d->characters[d->pc.position[dim_y]][d->pc.position[dim_x]]))->position[dim_y], ((pc_t*)(d->characters[d->pc.position[dim_y]][d->pc.position[dim_x]]))->position[dim_x]);
  // printf("number of monsters in fucntion : %d\n\n\n", nummon);
  ...
  for (i = 1; i <= nummon; i++) {
    ...
    //   free(d->characters[p[dim_y]][p[dim_x]]);
    //   d->characters[p[dim_y]][p[dim_x]] = NULL;
    //   printf("Cell was freed and set to NULL");
    // }
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
    for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
      if (d->characters[p[dim_y]][p[dim_x]] != NULL){
        ...
        // monster_t *m = (monster_t*) d->characters[p[dim_y]][p[dim_x]];
        // if (m) {
        //     putchar(m->symbol);
        // }
...
static void smart_next_step(dungeon_t *d, pair_t from, pair_t to, pair_t *next)
{
  ...
  // uint8_t min = d->pc_distance[((monster_t *)c)->position[dim_y]   ][((monster_t *)c)->position[dim_x]   ];
  // (*next)[dim_y] = ((monster_t *)c)->position[dim_y];
  // (*next)[dim_x] = ((monster_t *)c)->position[dim_x];
  // if(min > d->pc_distance[((monster_t *)c)->position[dim_y] +1 ][((monster_t *)c)->position[dim_x] +1 ]){
  //   min = d->pc_distance[((monster_t *)c)->position[dim_y] +1 ][((monster_t *)c)->position[dim_x] +1 ];
  //   (*next)[dim_y] += 1;
  //   (*next)[dim_x] += 1;
  // }
  // if(min > d->pc_distance[((monster_t *)c)->position[dim_y]    ][((monster_t *)c)->position[dim_x] +1 ]){
  //   min = d->pc_distance[((monster_t *)c)->position[dim_y]    ][((monster_t *)c)->position[dim_x] +1 ];
  //   (*next)[dim_y] += 0;
  //   (*next)[dim_x] += 1;
  // }
  // if(min > d->pc_distance[((monster_t *)c)->position[dim_y] +1 ][((monster_t *)c)->position[dim_x]    ]){
  //   min = d->pc_distance[((monster_t *)c)->position[dim_y] +1 ][((monster_t *)c)->position[dim_x]    ];
  //   (*next)[dim_y] += 1;
  //   (*next)[dim_x] += 0;
  // }
  // if(min > d->pc_distance[((monster_t *)c)->position[dim_y] -1 ][((monster_t *)c)->position[dim_x] +1 ]){
  ...
  while ((c = heap_remove_min(&h))) {
    if(c != NULL){
      ...
      if (((pc_t *)c)->sequence_num == 0) {
        ...
        //printf("Checkpoint for PC\n");

        // adding PC back to the queue
        //((pc_t *)c)->next_turn += 1000 / ((pc_t *)c)->speed;
        d->pc.next_turn += 1000 / d->pc.speed;
        //((pc_t *)d->characters[y][x])->hn = heap_insert(&h, c);
        d->pc.hn = heap_insert(&h, &d->pc);
        //printf("PC next turn: %d\n", ((pc_t *)c)->next_turn);

        //printf("Checkpoint for PC 2\n");
      } else if(((monster_t *)c)->position[dim_y] >= 0 && ((monster_t *)c)->position[dim_y] < DUNGEON_Y && ((monster_t *)c)->position[dim_x] >= 0 && ((monster_t *)c)->position[dim_x] < DUNGEON_X) {
        //-----------------------------------------------------------Monster
     
        
        // first handle erratic behavior
        ...
        pair_t next_move;
        if((monster_is(((monster_t *)c), ERRATIC)) && (rand() % 2)){
        ...
        // monster remaining still or moving
        if(mappair(next_move) > ter_floor){
          ...
          // if the rock hardness <= 85
          if(d->hardness[next_move[dim_y]][next_move[dim_x]] <= 85){
        ...
        //printf("Checkpoint 2\n");
        //printf("----------------------------Monster Check 2\n");
        

        // adding monster back to the queue
        ((monster_t *)c)->next_turn += 1000 / ((monster_t *)c)->speed;
        ((monster_t *)d->characters[y][x])->hn = heap_insert(&h, c);
        //printf("Monster next turn: %d\n", ((monster_t *)c)->next_turn);
        //printf("----------------------------Monster Check 3\n");
